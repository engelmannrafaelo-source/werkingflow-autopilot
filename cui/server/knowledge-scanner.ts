...
export async function performFullScan(autoAssign: boolean): Promise<ScanResult> {
  ...
  try {
    ...
    const validDocuments = documentsWithSummaries.filter(Boolean) as any[];

    // 5. Batch classify via AI Bridge
    console.log('[Scanner] Classifying documents via AI Bridge...');
    const classifications = await batchClassifyDocuments(
      validDocuments.map((d) => ({
        path: d.path,
        summary: d.summary,
        filename: d.filename,
        category: d.category,
    ...
    for (const doc of validDocuments) {
      ...
      const docKnowledge: DocumentKnowledge = registry.documents[doc.path] || {
        path: doc.path,
      ...
      for (const assignment of classification.assignments) {
        if (autoAssign && assignment.confidence >= 80) {
          ...
          docKnowledge.assignments = docKnowledge.assignments.filter(
            (a) => !(a.persona_id === assignment.persona_id && a.assigned_by === 'ai')
          );
      ...
      if (!docKnowledge.change_history) docKnowledge.change_history = [];
...
async function scanBusinessFolder(): Promise<
  Array<{
    path: string;
...
function extractCategory(relativePath: string): DocumentCategory {
...
function extractSmartSummary(content: string): string {
...
function parsePersonaProfile(id: string, content: string): PersonaProfile | null {
  ...
  if (primaryPathsSection) {
    ...
    if (pathLines) {
      pathLines.forEach((line) => {
        ...
        if (pathMatch) primaryPaths.push(pathMatch[1]);
      });
    }
  }
...
function updatePersonaKnowledge(
...
function updateScanStats(registry: KnowledgeRegistry): void {
...
async function loadRegistry(): Promise<KnowledgeRegistry> {
  try {
...
async function saveRegistry(registry: KnowledgeRegistry): Promise<void> {
