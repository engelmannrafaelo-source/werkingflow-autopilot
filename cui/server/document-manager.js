...
async function scanBusinessDocs(basePath) {
    const docs = [];
...
async function validateBusinessDocEdit(docPath, originalContent, proposedContent) {
    const errors = [];
    const warnings = [];
    ...
    if (docPath.includes('FEATURES.md')) {
        ...
        try {
            ...
            const registryContent = await fs_1.promises.readFile(registryPath, 'utf-8');
...
router.get('/documents/read', async (req, res) => {
    try {
        ...
        const docPath = path_1.default.join('/root/projekte/werkingflow/business', relativePath);
...
router.post('/documents/edit', async (req, res) => {
    try {
        ...
        const originalContent = await fs_1.promises.readFile(docPath, 'utf-8');
        // Validation
        ...
        // Diff erstellen
        const diff = (0, diff_1.createPatch)(relativePath, originalContent, proposedContent, 'Original', 'Proposed by ' + personaId);
        const edit = {
...
router.post('/reviews/:id/approve', async (req, res) => {
    try {
        ...
        // Apply Patch
        await fs_1.promises.writeFile(edit.documentPath, edit.proposedContent, 'utf-8');
        edit.status = 'approved';
        edit.updatedAt = new Date().toISOString();
        // Git Commit
        const relPath = edit.documentPath.replace('/root/projekte/werkingflow/', '');
        const commitMsg = `docs: ${edit.personaId} updated ${path_1.default.basename(edit.documentPath)}
