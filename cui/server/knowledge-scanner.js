...
exports.performIncrementalScan = performIncrementalScan;
exports.loadPersonaProfiles = loadPersonaProfiles;
const fs_1 = require("fs");
const path_1 = require("path");
const knowledge_classifier_js_1 = require("./knowledge-classifier.js");
...
async function performFullScan(autoAssign) {
    ...
    try {
        ...
        const documentsWithSummaries = await Promise.all(documents.map(async (doc) => {
            try {
                const content = await fs_1.promises.readFile(doc.absolute_path, 'utf-8');
                return {
                    ...doc,
        ...
        const classifications = await (0, knowledge_classifier_js_1.batchClassifyDocuments)(validDocuments.map((d) => ({
            path: d.path,
            summary: d.summary,
            filename: d.filename,
            category: d.category,
...
async function performIncrementalScan(files, autoAssign) {
    const startTime = Date.now();
    const result = {
    ...
    try {
        ...
        for (const relativePath of files) {
            const absolutePath = (0, path_1.join)(BUSINESS_PATH, relativePath);
            try {
        ...
        // Classify
        const classifications = await (0, knowledge_classifier_js_1.batchClassifyDocuments)(documents.map((d) => ({
            path: d.path,
            summary: d.summary,
            filename: d.filename,
            category: d.category,
...
async function loadPersonaProfiles() {
    const files = await fs_1.promises.readdir(PERSONAS_PATH);
    const profiles = [];
    for (const file of files) {
        if (!file.endsWith('.md'))
            continue;
        const personaId = file.replace('.md', '').split('-')[0]; // 'max-weber.md' → 'max'
        const content = await fs_1.promises.readFile((0, path_1.join)(PERSONAS_PATH, file), 'utf-8');
...
async function loadRegistry() {
    try {
        const content = await fs_1.promises.readFile(REGISTRY_PATH, 'utf-8');
