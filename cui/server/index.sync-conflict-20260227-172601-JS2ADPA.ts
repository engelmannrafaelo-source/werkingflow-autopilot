...
function sseProxy(targetBase: string, req: IncomingMessage, res: ServerResponse, cuiId?: string) {
  ...
  const proxyReq = httpRequest(url, { method: req.method, headers }, (proxyRes) => {
    ...
    proxyRes.on('data', (chunk: Buffer) => {
      chunkCount++;
      if (cuiId) {
        if (isForceStoppedAccount(cuiId)) return;
        ...
        if (attention) {
          ...
          // Any data chunk = Claude is working → keep state as working
          setSessionState(cuiId, cuiId, 'working');
          if (chunkCount % 5 === 1) {
            broadcast({ type: 'cui-data-activity', cuiId });
          }
        }
      }
    });
...
// --- Force-Stop Registry: prevents binary from overriding stopped state ---
// Key: accountId, Value: timestamp when force-stopped
const forceStoppedAccounts = new Map<string, number>();
const FORCE_STOP_TTL = 30_000; // 30s — ignore binary state updates for this long after stop

// Active monitor streams per account (so we can kill them on stop)
const activeMonitors = new Map<string, { req: ReturnType<typeof httpRequest>; timeout: ReturnType<typeof setTimeout> }>();

function isForceStoppedAccount(accountId: string): boolean {
  const ts = forceStoppedAccounts.get(accountId);
  if (!ts) return false;
  if (Date.now() - ts > FORCE_STOP_TTL) {
    forceStoppedAccounts.delete(accountId);
    return false;
  }
  return true;
}

// Cleanup expired force-stop entries every 60s
setInterval(() => {
  const now = Date.now();
  for (const [k, ts] of forceStoppedAccounts) {
    if (now - ts > FORCE_STOP_TTL) forceStoppedAccounts.delete(k);
  }
}, 60_000);
...
function monitorStream(targetBase: string, streamingId: string, cuiId: string, authHeaders: Record<string, string>) {
  ...
  // Kill previous monitor for this account if still active
  const prev = activeMonitors.get(cuiId);
  if (prev) { prev.req.destroy(); clearTimeout(prev.timeout); }

  const monitorReq = httpRequest(url, { method: 'GET', headers }, (monitorRes) => {
    if (monitorRes.statusCode !== 200) {
      console.log(`[Monitor] ${cuiId}: stream ${streamingId.slice(0,8)} returned ${monitorRes.statusCode}`);
    ...
    monitorRes.on('data', (chunk: Buffer) => {
      ...
      // Check for specific attention markers first
      const attention = detectAttentionMarkers(text);
      if (attention) {
        if (attention.state === 'idle') {
          console.log(`[Monitor] ${cuiId}: stream done (${chunkCount} chunks)`);
          broadcast({ type: 'cui-response-ready', cuiId });
          broadcast({ type: 'cui-state', cuiId, state: 'done' });
          setSessionState(cuiId, cuiId, 'idle', 'done');
          monitorReq.destroy();
          activeMonitors.delete(cuiId);
          return;
        }
      ...
      // ANY chunk = Claude is working → broadcast working state + trigger frontend poll
      // This is critical: tool_use, tool_result, text chunks all mean "actively working"
      setSessionState(cuiId, cuiId, 'working');
      // Broadcast a data-activity event so frontend knows to poll for new messages
      if (chunkCount % 3 === 1) {
        // Every 3rd chunk, nudge the frontend to refresh messages
        broadcast({ type: 'cui-data-activity', cuiId });
      }
    });

    monitorRes.on('end', () => {
      console.log(`[Monitor] ${cuiId}: stream ended (${chunkCount} chunks)`);
      if (!isForceStoppedAccount(cuiId)) {
        broadcast({ type: 'cui-response-ready', cuiId });
        broadcast({ type: 'cui-state', cuiId, state: 'done' });
        setSessionState(cuiId, cuiId, 'idle', 'done');
      }
  ...
  monitorReq.on('error', (err) => {
    console.log(`[Monitor] ${cuiId}: stream error: ${err.message}`);
    setTimeout(() => {
      if (isForceStoppedAccount(cuiId)) return;
      broadcast({ type: 'cui-state', cuiId, state: 'done' });
      broadcast({ type: 'cui-response-ready', cuiId });
  ...
  // Safety timeout: 10 minutes (was 45s — way too short for complex tasks)
  const safetyTimeout = setTimeout(() => {
    const current = sessionStates.get(cuiId);
    if (current?.state === 'working') {
      console.log(`[Monitor] ${cuiId}: 10min safety timeout, setting idle`);
      broadcast({ type: 'cui-state', cuiId, state: 'done' });
      broadcast({ type: 'cui-response-ready', cuiId });
      setSessionState(cuiId, cuiId, 'idle', 'done');
    }
  ...
  activeMonitors.set(cuiId, { req: monitorReq, timeout: safetyTimeout });
  console.log(`[Monitor] ${cuiId}: started for stream ${streamingId.slice(0,8)}`);
}
...
function setSessionState(key: string, accountId: string, state: ConvAttentionState, reason?: AttentionReason, sessionId?: string) {
  // Block non-idle state updates while account is force-stopped (prevents binary from overriding stop)
  if (state !== 'idle' && isForceStoppedAccount(accountId)) {
    console.log(`[State] Blocked ${accountId} → ${state}/${reason ?? '-'} (force-stopped)`);
    return;
  }
...
app.get('/api/mission/conversations', async (req, res) => {
  ...
  // Dead stream detection: fix stale "ongoing" status for conversations with dead streams
  for (const conv of deduped) {
    if (conv.status === 'ongoing' && !activeMonitors.has(conv.accountId)) {
      const currentState = sessionStates.get(conv.accountId);
      if (!currentState || currentState.state === 'idle') {
        conv.status = 'completed';
        conv.streamingId = null;
      }
    }
  }
...
app.get('/api/mission/conversation/:accountId/:sessionId', async (req, res) => {
  ...
  // Smart status detection: binary may report "ongoing" for dead/stuck conversations
  const binaryStatus = convResp.data.metadata?.status || convResp.data.status || 'completed';
  const forceStopped = isForceStoppedAccount(req.params.accountId);
  let effectiveStatus = forceStopped ? 'completed' : binaryStatus;

  // Dead stream detection: if binary says "ongoing" but no active monitor exists,
  // the stream died silently. Report as completed so UI doesn't show stale "working".
  if (effectiveStatus === 'ongoing' && !activeMonitors.has(req.params.accountId)) {
    const currentState = sessionStates.get(req.params.accountId);
    if (!currentState || currentState.state === 'idle') {
  ...
  res.json({
    ...
    permissions: forceStopped ? [] : sessionPermissions, // Clear permissions when force-stopped
    totalMessages: rawMessages.length,
    isAgentDone: forceStopped ? true : isAgentDone,
    rateLimited,
...
app.post('/api/mission/send', async (req, res) => {
  ...
  // Clear force-stop flag when user sends a new message (user wants to resume)
  if (forceStoppedAccounts.has(effectiveAccountId)) {
    console.log(`[Send] Clearing force-stop for ${effectiveAccountId} (user resumed)`);
    forceStoppedAccounts.delete(effectiveAccountId);
  }
...
app.get('/api/mission/states', (_req, res) => {
  const states = getSessionStates();
  // Override force-stopped accounts to always show idle
  for (const [key, state] of Object.entries(states)) {
    if (isForceStoppedAccount(state.accountId) && state.state !== 'idle') {
      states[key] = { ...state, state: 'idle', reason: 'done' };
    }
  }
  res.json(states);
});
...
app.post('/api/mission/start', async (req, res) => {
  ...
  // Clear force-stop flag when starting a new conversation
  if (forceStoppedAccounts.has(effectiveAccountId)) {
    console.log(`[Start] Clearing force-stop for ${effectiveAccountId} (new conversation)`);
    forceStoppedAccounts.delete(effectiveAccountId);
  }
...
// 7. Stop conversation (with nuclear child-process kill + force-stop state)
const ACCOUNT_PM2_MAP: Record<string, string> = { rafael: 'cui-1', engelmann: 'cui-2', office: 'cui-3' };

// Helper: kill all child processes of a PM2-managed binary
function killBinaryChildren(accountId: string): number {
  ...
  try {
    ...
    console.log(`[Stop] ${accountId}: killed ${uniquePids.length} child processes of ${pmName} (PID ${binaryPid})`);
...
app.post('/api/mission/conversation/:accountId/:sessionId/stop', async (req, res) => {
  ...
  console.log(`[Stop] ▶ ${accountId}/${sessionId.slice(0,8)}: Force-stop initiated`);

  // 1. Set force-stop flag FIRST — blocks all state updates from binary/monitor
  forceStoppedAccounts.set(accountId, Date.now());

  // 2. Kill active monitor stream (prevents state override from SSE events)
  const monitor = activeMonitors.get(accountId);
  if (monitor) {
    monitor.req.destroy();
    clearTimeout(monitor.timeout);
    activeMonitors.delete(accountId);
    console.log(`[Stop] ${accountId}: monitor stream destroyed`);
  }

  // 3. Set session state to idle immediately
  setSessionState(accountId, accountId, 'idle', 'done');
  broadcast({ type: 'cui-state', cuiId: accountId, state: 'done' });
  broadcast({ type: 'cui-response-ready', cuiId: accountId });

  // 4. Send stop to Claude Code binary (first attempt)
  const resp = await cuiFetch(port, `/api/conversations/${sessionId}/stop`, { method: 'POST' });
  console.log(`[Stop] ${accountId}/${sessionId.slice(0,8)}: API stop #1 ${resp.ok ? 'OK' : 'FAIL (' + resp.error + ')'}`);

  // 5. Nuclear kill: terminate all child processes
  const killed = killBinaryChildren(accountId);

  // 6. Schedule repeated stop signals — catches binary recovering and starting new turns
  //    Send stop every 2s for 10s total (5 retries)
  let retryCount = 0;
  const stopRetryInterval = setInterval(async () => {
    retryCount++;
    if (retryCount >= 5) { clearInterval(stopRetryInterval); return; }
    try {
      const retryResp = await cuiFetch(port, `/api/conversations/${sessionId}/stop`, { method: 'POST' });
      const retryKilled = killBinaryChildren(accountId);
      console.log(`[Stop] ${accountId}/${sessionId.slice(0,8)}: retry #${retryCount} stop=${retryResp.ok ? 'OK' : 'FAIL'} killed=${retryKilled}`);
    } catch { /* ignore */ }
  }, 2000);

  res.json({ ...(resp.data || {}), stopped: true, childrenKilled: killed, forceStopUntil: Date.now() + FORCE_STOP_TTL });
});
...
app.get('/api/admin/wr/users', async (req, res) => {
  try {
    ...
    const r = await wrProxy(`${wrBase()}/api/admin/users${qs ? '?' + qs : ''}`);
...
app.post('/api/admin/wr/users/bulk', async (req, res) => {
  try {
    const r = await wrProxy(`${wrBase()}/api/admin/users/bulk`, {
      method: 'POST',
...
app.patch('/api/admin/wr/tenants/:id', async (req, res) => {
  try { const r = await wrProxy(`${wrBase()}/api/admin/tenants/${req.params.id}`, { method: 'PATCH', body: JSON.stringify(req.body) }); res.status(r.status).json(r.body); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/admin/wr/activity-stream', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      res.status(response.status).json({ error: `Failed to connect to activity stream: ${response.statusText}` });
      return;
    }
    ...
    // Pipe the SSE stream from WR to the client
    if (response.body) {
      ...
      const streamToClient = async () => {
        try {
          ...
          console.error('[Activity Stream Proxy] Stream error:', error);
        } finally {
          res.end();
        }
      };
    ...
    console.error('[Activity Stream Proxy] Error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/infrastructure/app/:id/restart', async (req, res) => {
  const { id } = req.params;
  try {
...
app.post('/api/infrastructure/app/:id/test/restart', async (req, res) => {
  const { id } = req.params;
  try {
