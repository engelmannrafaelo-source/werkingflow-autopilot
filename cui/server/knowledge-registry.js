...
const migrate_freshness_index_js_1 = require("./migrate-freshness-index.js");
const knowledge_scanner_js_1 = require("./knowledge-scanner.js");
...
router.post('/scan', async (req, res) => {
    try {
        ...
        if (mode === 'full') {
            const result = await (0, knowledge_scanner_js_1.performFullScan)(auto_assign);
            res.json(result);
        }
            ...
            const result = await (0, knowledge_scanner_js_1.performIncrementalScan)(files, auto_assign);
            res.json(result);
        }
        else {
...
router.post('/assign', async (req, res) => {
    try {
        ...
        try {
            await fs_1.promises.access(personaFile);
        }
        catch {
...
router.post('/migrate-freshness-index', async (req, res) => {
    try {
        const result = await (0, migrate_freshness_index_js_1.migrateFreshnessIndex)();
        res.json({
            success: true,
            ...result,
        });
