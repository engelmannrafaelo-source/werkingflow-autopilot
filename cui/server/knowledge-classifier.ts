...
export async function batchClassifyDocuments(
...
): Promise<Record<string, ClassificationResponse>> {
  const results: Record<string, ClassificationResponse> = {};
  const BATCH_SIZE = 10;
  ...
  for (let i = 0; i < documents.length; i += BATCH_SIZE) {
    ...
    console.log(
      `[Classifier] Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(documents.length / BATCH_SIZE)}`
    );
...
function buildClassificationUserPrompt(req: ClassificationRequest): string {
...
function parseClassificationResponse(content: string): ClassificationResponse {
  ...
  parsed.assignments = parsed.assignments.filter((a: any) => a.confidence >= 30);
