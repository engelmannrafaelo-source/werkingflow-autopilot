...
router.get('/persona/:id', async (req, res) => {
  try {
    ...
    const response: PersonaQueryResponse = {
...
router.get('/document', async (req, res) => {
  try {
    ...
    const registry = await loadRegistry();
    const doc = registry.documents[docPath as string];
...
router.post('/scan', async (req, res) => {
  try {
    const { mode, files, auto_assign = false }: ScanRequest = req.body;
...
router.post('/assign', async (req, res) => {
  try {
    const { document_path, persona_id, relevance, reason }: AssignRequest = req.body;
    ...
    registry.documents[document_path].assignments = registry.documents[
      document_path
    ].assignments.filter((a) => a.persona_id !== persona_id);
