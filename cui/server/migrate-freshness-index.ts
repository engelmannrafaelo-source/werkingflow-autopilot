...
interface FreshnessIndex {
  last_scan: string;
  documents: Record<string, FreshnessIndexDocument>;
  summary: {
    total: number;
    current: number;
    stale: number;
...
const OWNER_TO_PERSONA_MAP: Record<string, string> = {
  'rafael': 'rafael',
  'mira': 'mira',
  'vera': 'vera',
  'max': 'max',
...
export async function migrateFreshnessIndex(): Promise<MigrationResult> {
  ...
  try {
    ...
    const freshnessIndex: FreshnessIndex = JSON.parse(freshnessContent);
    ...
    for (const [docPath, freshnessDoc] of Object.entries(freshnessIndex.documents)) {
      ...
      try {
        ...
        const docKnowledge: DocumentKnowledge = {
          path: normalizedPath,
    ...
    await fs.copyFile(
      ...
      FRESHNESS_INDEX_PATH.replace('.json', '.backup.json')
    );
...
async function buildPersonaKnowledge(registry: KnowledgeRegistry): Promise<void> {
  // Build persona knowledge from document assignments
  const personaMap = new Map<string, PersonaKnowledge>();
