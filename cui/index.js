...
function sseProxy(targetBase, req, res, cuiId) {
...
function monitorStream(targetBase, streamingId, cuiId, authHeaders) {
...
function messagePostProxy(targetBase, cuiId, req, res) {
    const url = new URL(req.url, targetBase);
    const headers = {};
    for (const [k, v] of Object.entries(req.headers)) {
        if (v && typeof v === 'string')
...
function serveInjectedHtml(targetBase, req, res) {
    const url = new URL(req.url, targetBase);
    ...
    const fetchReq = httpRequest(url, { method: 'GET', headers }, (fetchRes) => {
        ...
        fetchRes.on('end', () => {
            ...
            for (const [k, v] of Object.entries(fetchRes.headers)) {
                if (v && k !== 'content-length' && k !== 'content-encoding')
                    resHeaders[k] = v;
            }
...
for (const cui of CUI_PROXIES) {
    ...
    proxy.on('error', (err, req, res) => {
        ...
        broadcast({ type: 'cui-state', cuiId: cui.id, state: 'error', message: err.message });
    ...
    proxyServer.on('upgrade', (req, socket, head) => {
        proxy.ws(req, socket, head, undefined, (err) => {
            console.error(`[Proxy ${cui.id}] WS upgrade error:`, err.message);
            socket.destroy();
        });
    });
...
function removePanelVisibility(projectId, panelId) {
...
function setSessionState(key, accountId, state, reason, sessionId) {
...
function detectAttentionMarkers(text) {
    ...
    ws.on('message', (raw) => {
        try {
            ...
            if (msg.type === 'state-report') {
                if (msg.activeProjectId)
                    workspaceState.activeProjectId = msg.activeProjectId;
                if (msg.panels)
                    workspaceState.panels = msg.panels;
            }
...
function broadcast(data) {
    ...
    if (data.type === 'cui-state' && data.cuiId && data.state) {
        workspaceState.cuiStates[data.cuiId] = data.state;
    }
...
app.get('/api/files', async (req, res) => {
    ...
    if (isRemotePath(dirPath)) {
        try {
            ...
            const raw = await sshExec(`mkdir -p ${dirPath} && find ${dirPath} -maxdepth 1 -not -name '.*' -not -path '${dirPath}' -printf '%y\\t%f\\n' 2>/dev/null | sort -t$'\\t' -k1,1r -k2,2`);
...
function autoTitleFromSummary(summary) {
    if (!summary)
        return '';
    // Take first line, clean up
    let title = summary.split('\n')[0].replace(/\s+/g, ' ').trim();
    // Skip unhelpful summaries
    if (title.startsWith('API Error') || title.startsWith('{') || title.startsWith('Error:'))
        return '';
...
function autoTitleUntitled(results) {
...
function saveAssignment(sessionId, accountId) {
    const assignments = loadAssignments();
    if (assignments[sessionId] === accountId)
        return; // No change
    assignments[sessionId] = accountId;
    writeFileSync(ASSIGNMENTS_FILE, JSON.stringify(assignments, null, 2));
}
function getAssignment(sessionId) {
...
function loadFinished() {
    if (!existsSync(FINISHED_FILE))
        return {};
    try {
        return JSON.parse(readFileSync(FINISHED_FILE, 'utf8'));
    }
    catch {
        return {};
    }
}
function setFinished(sessionId, finished) {
...
let _lastPromptCache = null;
function loadLastPrompt() {
    if (_lastPromptCache)
        return _lastPromptCache;
    if (!existsSync(LAST_PROMPT_FILE)) {
        _lastPromptCache = {};
        return _lastPromptCache;
    }
    try {
        _lastPromptCache = JSON.parse(readFileSync(LAST_PROMPT_FILE, 'utf8'));
        return _lastPromptCache;
    }
    catch {
        _lastPromptCache = {};
...
function setLastPrompt(sessionId) {
...
function deduplicateConversations(results) {
    const assignments = loadAssignments();
...
async function cuiFetch(proxyPort, path, options) {
...
function resolveProjectName(projectPath) {
    ...
    for (const f of projects) {
        try {
            ...
            if (p.workDir && projectPath.includes(p.id))
                return p.name;
        }
        catch { /* skip */ }
    }
...
function getProxyPort(accountId) {
...
app.get('/api/mission/conversations', async (req, res) => {
    ...
    await Promise.all(CUI_PROXIES.map(async (proxy) => {
        ...
        for (const c of data.conversations) {
            if (filterProject && !c.projectPath?.includes(filterProject))
                continue;
            results.push({
    ...
    for (const conv of deduped) {
        ...
        if (stateInfo) {
            conv.attentionState = stateInfo.state;
            conv.attentionReason = stateInfo.reason;
        }
    }
    ...
    for (const conv of deduped) {
        if (finished[conv.sessionId]) {
            conv.manualFinished = true;
        }
    }
    // Enrich with visibility status
    const visibleIds = getVisibleSessionIds();
    for (const conv of deduped) {
        conv.isVisible = visibleIds.has(conv.sessionId);
    }
...
app.get('/api/mission/conversation/:accountId/:sessionId', async (req, res) => {
    ...
    if (!convData) {
        res.status(502).json({ error: 'CUI unreachable' });
        return;
    }
    // Transform CUI message format: {type, message: {role, content}, timestamp} → {role, content, timestamp}
    const rawMessages = (convData.messages || []).sort((a, b) => (a.timestamp || '').localeCompare(b.timestamp || ''));
    const messages = rawMessages.slice(-tail).map((m) => ({
...
app.delete('/api/mission/conversation/:sessionId', (req, res) => {
    ...
    if (prompts[sid]) {
        delete prompts[sid];
        writeFileSync(LAST_PROMPT_FILE, JSON.stringify(prompts, null, 2));
    }
...
app.post('/api/mission/activate', (req, res) => {
    ...
    const projectsData = projectFiles.map(f => {
        try {
            return JSON.parse(readFileSync(join(PROJECTS_DIR, f), 'utf8'));
        }
        catch {
            return null;
        }
...
app.post('/api/mission/auto-titles', async (_req, res) => {
    ...
    await Promise.all(CUI_PROXIES.map(async (proxy) => {
        ...
        if (!data?.conversations)
            return;
        for (const c of data.conversations) {
            if (c.sessionInfo?.custom_name || getTitle(c.sessionId))
    ...
    for (const conv of allConvs) {
        try {
            ...
            const firstUserMsg = detail.messages.find((m) => (m.type === 'user' || m.message?.role === 'user'));
            if (!firstUserMsg)
                continue;
    ...
    try {
        ...
        await Promise.all(CUI_PROXIES.map(async (proxy) => {
            ...
            for (const c of data.conversations) {
                ...
                const lastMsgs = rawMsgs.slice(-3).map((m) => ({
                    role: m.message?.role || m.type || 'user',
                    content: typeof m.message?.content === 'string' ? m.message.content.slice(0, 300) :
                        Array.isArray(m.message?.content) ? m.message.content.filter((b) => b.type === 'text').map((b) => b.text).join(' ').slice(0, 300) : '',
                }));
                conversations.push({
                    sessionId: c.sessionId,
                    accountId: proxy.id,
        ...
        for (const p of projects) {
            if (!p.workDir || !isRemotePath(p.workDir))
                continue;
            try {
...
async function getCommanderContext() {
    if (_ctxCache && Date.now() - _ctxCache.ts < CTX_CACHE_TTL)
...
Antworte auf Deutsch, präzise und kompakt.`;
    if (context) {
        try {
            ...
            const active = (ctxData.conversations || []).filter((c) => c.status === 'ongoing');
    ...
    if (!existsSync(sharedPath)) {
        ...
        if (existsSync(credPath)) {
            try {
                ...
                for (const [_appId, appData] of Object.entries(creds)) {
                    md += `## ${appData.name}`;
                    if (appData.productionUrl)
                        md += ` — [${appData.productionUrl}](${appData.productionUrl})`;
                    md += `\n\n`;
...
app.post('/api/images', async (req, res) => {
    const { images, accountId } = req.body;
    if (!images?.length) {
...
app.get('/api/team/personas', async (_req, res) => {
    ...
    try {
        ...
        const personas = await Promise.all(personaFiles.map(async (file) => {
...
app.get('/api/workspaces/:workspace/credentials', async (req, res) => {
    try {
        ...
        const safeRegistry = {
            ...
            personas: Object.entries(registry.personas || {}).map(([key, persona]) => ({
                key,
                email: persona.email,
                role: persona.role,
            ...
            expert_reviewers: Object.entries(registry.expert_reviewers || {}).map(([key, reviewer]) => ({
                key,
...
app.post('/api/workspaces/:workspace/credentials/validate', async (req, res) => {
    try {
        ...
        const registry = JSON.parse(readFileSync(registryPath, 'utf-8'));
        const personaExists = ((registry.personas && registry.personas[persona]) ||
        ...
        const personaData = registry.personas?.[persona] ||
...
function parsePersona(filename, content) {
    ...
    return {
        ...
        governance: governanceMatch?.[1]?.trim(),
        reportsTo: reportsToMatch?.[1]?.trim() || null,
    };
}
let tasks = []; // In-Memory für MVP - später DB
// GET /api/team/tasks
...
app.get('/api/team/chat/:personaId/history', async (req, res) => {
    ...
    try {
        ...
        const response = await fetch(`${BRIDGE_URL}/v1/sessions/${sessionId}`, { headers: { Authorization: `Bearer ${BRIDGE_KEY}` } });
        if (!response.ok) {
            return res.json({ messages: [] });
        }
...
app.post('/api/files/move', async (req, res) => {
    const { sourcePath, targetDir, operation } = req.body;
...
changeWatcher.on('error', (err) => {
    console.warn('[ChangeWatch] Watcher error:', err.message);
});
changeWatcher.on('all', (event, filePath) => {
    if (!/\.(ts|tsx|css|html|json)$/.test(filePath))
        return;
    ...
    if (!_pendingChanges.includes(rel))
        _pendingChanges.push(rel);
...
app.post('/api/control/cui/reload', (req, res) => {
    ...
    if (!cuiId) {
        res.status(400).json({ error: 'cuiId required' });
        return;
    }
...
app.post('/api/control/cui/cwd', (req, res) => {
    ...
    if (!cuiId || !cwd) {
        res.status(400).json({ error: 'cuiId and cwd required' });
        return;
    }
...
function saveWrEnvMode(mode) {
    try {
        writeFileSync(WR_ENV_MODE_FILE, JSON.stringify({ mode }));
    }
    catch { }
}
let wrEnvMode = loadWrEnvMode();
function wrBase() { return wrEnvMode === 'staging' ? WR_STAGING_URL : WR_PROD_URL; }
console.log(`[WR Env] Loaded mode: ${wrEnvMode} → ${wrBase()}`);
function wrAdminHeaders() {
    if (!WR_ADMIN_SECRET)
        throw new Error('WERKING_REPORT_ADMIN_SECRET not set');
    ...
    try {
        const results = await Promise.all(VERCEL_APPS.map(async (app) => {
            try {
                ...
                if (!dep)
                    return { name: app.name, state: 'UNKNOWN' };
                return {
                    name: app.name,
...
app.post('/api/control/snapshot/request', (req, res) => {
    ...
    if (!panel) {
        res.status(400).json({ error: 'panel required' });
        return;
    }
...
const panelScreenshots = new Map();
...
app.post('/api/screenshot/:panel', (req, res) => {
    ...
    const meta = { panel, capturedAt: new Date().toISOString(), width: width ?? 0, height: height ?? 0, filePath };
...
app.get('/api/screenshot/:panel', (req, res) => {
    ...
    const meta = panelScreenshots.get(panel);
    if (!meta) {
        res.status(404).json({ error: 'No screenshot for panel: ' + panel });
        return;
    }
...
app.get('/api/dev/screenshot-live', async (req, res) => {
    ...
    const wait = Math.min(parseInt(req.query.wait) || 3000, 15000);
    if (!panelType && !nodeId) {
        ...
        if (panelExists === 0) {
            ...
            if (panelType && !nodeId) {
                ...
                await page.evaluate((pt) => {
                    const select = document.querySelector('select[title="Tab hinzufuegen"]');
                    if (select) {
                        select.value = pt;
        ...
        const panels = await page.evaluate(() => {
            ...
            for (const panel of allPanels) {
                ...
                for (const tab of tabs) {
                    const tabContent = tab.closest('.flexlayout__tabset')?.querySelector(`[data-node-id="${nodeId}"]`);
                    if (tabContent) {
                        tabName = tab.querySelector('.flexlayout__tab_button_content')?.textContent?.trim() || null;
                        break;
                    }
                }
                if (nodeId) {
...
async function agentReadJsonlLastN(filePath, n) {
    try {
...
app.get('/api/agents/status', async (_req, res) => {
    const agents = await Promise.all(Object.entries(AGENT_REGISTRY).map(async ([id, info]) => {
        ...
        let inboxCount = 0;
        try {
            const inbox = await fsAgentPromises.readFile(`${AGENTS_DIR}/inbox/${info.persona_id}.md`, 'utf-8');
            inboxCount = inbox.split('---').filter(s => s.trim()).length;
        }
        catch { /**/ }
        let approvalsCount = 0;
        try {
            const raw = await fsAgentPromises.readFile(`${AGENTS_DIR}/approvals/pending.jsonl`, 'utf-8');
            approvalsCount = raw.trim().split('\n').filter(Boolean).filter(l => { try {
                return JSON.parse(l).persona?.toLowerCase().includes(id.toLowerCase());
            }
            catch {
                return false;
            } }).length;
        ...
        if (runningAgents.has(id))
            status = 'working';
        else if (last?.response_preview?.startsWith('ERROR:'))
            status = 'error';
        const now = new Date();
...
app.get('/api/agents/approvals', async (_req, res) => {
    try {
        const raw = await fsAgentPromises.readFile(`${AGENTS_DIR}/approvals/pending.jsonl`, 'utf-8');
        const approvals = raw.trim().split('\n').filter(Boolean).map((l, i) => { try {
            return { index: i, ...JSON.parse(l) };
        }
        catch {
            return null;
        } }).filter(Boolean);
...
app.post('/api/agents/approve', async (req, res) => {
    ...
    try {
        ...
        if (execute && entry.type === 'bash')
            execAsync(entry.payload, { cwd: AGENTS_DIR, timeout: 30000 }).then(({ stdout, stderr }) => console.log('[Approval] OK:', stdout || stderr)).catch(e => console.error('[Approval]', e.message));
...
app.post('/api/agents/trigger/:id', (req, res) => {
    ...
    proc.stdout?.on('data', (d) => console.log(`[Agent:${id}]`, d.toString().trim()));
    proc.stderr?.on('data', (d) => console.error(`[Agent:${id}]`, d.toString().trim()));
...
app.get('/api/agents/claude/status', async (_req, res) => {
    const readSafe = async (p, fb = '') => { try {
        return await fsAgentPromises.readFile(p, 'utf-8');
    }
    catch {
        return fb;
    } };
    const agents = await Promise.all(Object.entries(CLAUDE_AGENT_REGISTRY).map(async ([id, info]) => {
        ...
        try {
            ...
            last_outcome = (last.outcome ?? '').slice(0, 100);
        }
        catch { /**/ }
        let inbox_count = 0;
        try {
            inbox_count = ((await fsAgentPromises.readFile(`${AGENTS_DIR}/inbox/${id}.md`, 'utf-8')).match(/^---$/gm) ?? []).length;
        }
        catch { /**/ }
        return { id, name: info.name, schedule: info.schedule, task_type: info.task_type, status: runningClaudes.has(id) ? 'working' : 'idle', last_run, last_outcome, inbox_count };
    }));
    res.json({ agents });
});
app.post('/api/agents/claude/run', async (req, res) => {
    const { persona_id, task, mode, plan_id } = req.body;
    const runMode = mode ?? 'plan'; // Default: plan first
    if (!CLAUDE_AGENT_REGISTRY[persona_id])
        return res.status(404).json({ error: `Unknown persona: ${persona_id}` });
    if (runningClaudes.has(persona_id))
    ...
    const writeLog = (s) => fsAgentPromises.appendFile(logFile, s).catch(() => { });
    ...
    proc.stdout?.on('data', (d) => writeLog(d.toString()));
...
app.get('/api/agents/claude/log/:taskId', async (req, res) => {
    ...
    try {
        res.write(`data: ${JSON.stringify({ text: await fsAgentPromises.readFile(logFile, 'utf-8'), init: true })}\n\n`);
    }
    catch { /**/ }
    const tail = spawn('tail', ['-f', '-n', '0', logFile]);
    tail.stdout?.on('data', (d) => res.write(`data: ${JSON.stringify({ text: d.toString() })}\n\n`));
...
app.get('/api/agents/claude/memory/:personaId', async (req, res) => {
    ...
    const summary = await (async () => { try {
        return await fsAgentPromises.readFile(`${AGENTS_DIR}/memory/${id}.summary.md`, 'utf-8');
    }
    catch {
        return 'Kein Memory.';
    } })();
    const raw = await (async () => { try {
        return await fsAgentPromises.readFile(`${AGENTS_DIR}/memory/${id}.jsonl`, 'utf-8');
    }
    catch {
        return '';
    } })();
    const runs = raw.trim().split('\n').filter(Boolean).slice(-10).map(l => { try {
        return JSON.parse(l);
    }
    catch {
        return null;
...
app.post('/api/agents/business/approve', async (req, res) => {
    const { index, commit_message } = req.body;
    try {
        const raw = await fsAgentPromises.readFile(BUSINESS_QUEUE, 'utf-8');
        const lines = raw.trim().split('\n').filter(Boolean);
...
app.get('/api/agents/persona/:id', async (req, res) => {
    try {
        ...
        const persona = { id: req.params.id };
        // Parse header
...
app.post('/api/commands/run-agent', async (req, res) => {
    const { persona_id, task, test_mode } = req.body;
    try {
    ...
    try {
        ...
        res.json({
            agents: {
                ...
                working: agentsData.agents?.filter((a) => a.status === 'working').length || 0,
                idle: agentsData.agents?.filter((a) => a.status === 'idle').length || 0,
            },
            approvals: {
...
app.get('/api/claude-code/stats-v2', async (_req, res) => {
    try {
        ...
        Object.entries(ACCOUNT_MAP).forEach(([accountName, workspaceNames]) => {
            ...
            let status = 'safe';
            if (weeklyLimitPercent > 80)
                status = 'critical';
            else if (weeklyLimitPercent > 60)
                status = 'warning';
            accounts.push({
                accountName,
                ...
                status,
                nextWindowReset: nextWindowReset ? new Date(nextWindowReset).toISOString() : null,
